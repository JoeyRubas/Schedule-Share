...
def entry():
    ...
    if form.submit1.data and form.is_submitted():
        ...
        for num in range(8):
            try:
                schedule_ids.append(result["p"+str(num+1)])
                schedule_ids.append(convert[result["p"+str(num+1)]])
            except:
        ...
        data = {"id":result["fname"],
                 "fname":result["fname"],
                 "lname":result["lname"],
                 "school":result["school"],
                 "name":(result["fname"]+" "+result["lname"]).title(),
                 "schedule_ids":[result["p"+str(num+1)] for num in range(8)],
                 "schedule_names":[convert[result["p"+str(num+1)]] for num in range(8)]}
        mongo.db.users.insert(data)
        for clas in data["schedule_ids"]:
            if mongo.db.clas.find({'id': clas}).count() > 0:
                before = mongo.db.clas.find_one_or_404({"id":clas})
                before["students_id"].append(data["id"])
                before["students_name"].append(data["name"])
                mongo.db.clas.update_one({"id":clas}, {"$set":before})
...
@app.route("/edit/<string:id1>", methods=["get", "post"])
def edit(id1):
    ...
    if form.submit1.data and form.is_submitted():
        ...
        for num in range(8):
            if start["schedule_ids"][num] != data["schedule_ids"][num]:
                before = mongo.db.clas.find_one_or_404({"id":start["schedule_ids"][num]})
                del before["students_id"][before["students_id"].index(id1)]
                del before["students_name"][before["students_name"].index(start["name"])]
                mongo.db.clas.update_one({"id":start[clas]}, {"$set":before})

                                          
                before = mongo.db.clas.find_one_or_404({"id":data["schedule_ids"][num]})
                before["students_id"].append(data["id"])
                before["students_name"].append(data["name"])
                mongo.db.clas.update_one({"id":data["schedule_ids"][num]}, {"$set":before})
        return render_template("person.html",person = mongo.db.users.find_one_or_404({"id":data["id"]}), search = search)
